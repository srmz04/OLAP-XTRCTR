...
@dataclass
class Config:
    """Configuración centralizada - usa variables de entorno si disponibles"""
    server: str = None
    user: str = None
    password: str = None
    ...
    def __post_init__(self):
        ...
        self.server = self.server or os.environ.get('DGIS_SERVER', 'reportesdgis.salud.gob.mx')
        self.user = self.user or os.environ.get('DGIS_USER', 'PWIDGISREPORTES\\DGIS15')
        self.password = self.password or os.environ.get('DGIS_PASSWORD', 'Temp123!')
    ...
    def download_members_only(self, catalog: str) -> bool:
        ...
        try:
            ...
            with ConnectionManager(self.config, catalog) as conn:
                ...
                csv_filename = f"{catalog}_miembros_completos.csv"
                csv_path = Path(self.config.output_dir).parent / csv_filename
                members_df.to_csv(csv_path, index=False)
